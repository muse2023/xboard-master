;if(CloudflareApps.matchPage(CloudflareApps.installs['goSmgvteAcQ2'].URLPatterns)){(function(){'use strict'
...
var options=window.CloudflareApps.installs['goSmgvteAcQ2'].options
...
if("goSmgvteAcQ2"==='preview')updateVisibility()}
function updateVisibility(){clearTimeout(visibilityTimeout)
var visible=true
var stored=false
try{stored=JSON.parse(store['cf-flashcard'])}catch(e){}
if("goSmgvteAcQ2"==='preview'||!stored){visible=true}else{var timeframe=times[options.renew]
...
window.CloudflareApps.installs['goSmgvteAcQ2'].scope={updateOptions:function updateOptions(nextOptions){clearTimeout(visibilityTimeout)
options=nextOptions
updateContent()
